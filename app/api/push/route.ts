...
function isQuietNow(start: string | null, end: string | null, timeZone: string) {
  if (!start || !end) return false;
  const now = timeInZone(timeZone);
...
export async function POST(request: Request) {
  ...
  if (!client || !admin || !publicKey || !privateKey || !subject) return NextResponse.json({ error: "push_not_configured" }, { status: 503 });
  ...
  if (!body.resourceId || !["poke", "daily_answer", "challenge"].includes(body.eventType ?? "")) return NextResponse.json({ error: "invalid_request" }, { status: 400 });

  const { data: membership } = await admin.from("couple_members").select("couple_id").eq("user_id", auth.user.id).maybeSingle();
  if (!membership) return NextResponse.json({ error: "not_paired" }, { status: 403 });
  ...
  if (!partner) return NextResponse.json({ error: "partner_missing" }, { status: 409 });
  ...
  if (body.eventType === "poke") {
    ...
    if (!data) return NextResponse.json({ error: "invalid_resource" }, { status: 403 });
  ...
  if (preference?.fun_updates === false || isQuietNow(preference?.quiet_start ?? null, preference?.quiet_end ?? null, preference?.timezone || "Asia/Tehran")) return NextResponse.json({ sent: 0, skipped: true });
